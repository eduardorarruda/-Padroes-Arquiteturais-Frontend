...
//  Cadastro de Parceiros — Clientes e Fornecedores
// ============================================================

import React, { useState, useEffect, useCallback } from 'react';
import { PlusCircle, Trash2, Users, Building2, Search, UserCheck, Briefcase } from 'lucide-react';
...
// ── Listagem ───────────────────────────────────────────────────
function ListaParceiros({ parceiros, filtro, onDeletar }) {
                    ...
                    <tbody className="divide-y divide-slate-100">
                      ...
                      ${parceiro.tipo === 'Cliente'
                                ...
                                <td className="px-6 py-4 text-right">
                                    <button
                                        onClick={() => onDeletar(parceiro.id, parceiro.nome)}
...
export default function ParceirosPage() {
    ...
    const handleDeletar = async (id, nome) => {
        if (!window.confirm(`Deseja realmente excluir o parceiro "${nome}"?`)) return;
        try {
            await fetch(`/api/parceiros/${id}`, { method: 'DELETE' });
            toast.success(`Parceiro "${nome}" removido.`);
            carregar();
        } catch (err) {
        ...
        <div>
            ...
            <header className="mb-6">
                ...
                <p className="text-slate-500 mt-1 text-sm">
                    Gerencie seus clientes e fornecedores em um só lugar.
                </p>
            </header>
